  ...
  var reporting = reporting || {
    ...
    reports: function(opts, report, cb) {
      if ((!/object/.test(typeof report)) || (report.hasOwnProperty('code'))) {
        console.log(" LIBNMAP -> reports -> calling callback from random if statement");
      ...
      if (!opts.json) {
        console.log(" LIBNMAP -> reports -> calling callback from not json switch");
        return cb(null, xml);
      }
      ...
      try {
        ...
        xmlParser.parseString(xml, function parseXML(err, json) {
          if(err) {
            console.log(" LIBNMAP -> reports -> calling callback from xmlparser error");
            return cb(new Error(err));
          }

          console.log(" LIBNMAP -> reports -> calling callback from xmlparser");
          console.log(json.nmaprun);
          return cb(null, json.nmaprun);
        });
      } catch(err) {
        console.log(" LIBNMAP -> reports -> calling callback from catch");
        console.log(err);
          ...
          if (!adapter.internal) {
            ...
            if (adapter.netmask) {
              ...
              if (validation.test(validation.net.IPv6, netmask)) {
                ...
                //netmask = 64; // Force local host discovery
              }
        ...
        funcs[range] = function block(callback) {
          ...
          var report = []
            , execute = proc(cmd, function exe(err, stdout, stderr) {
              if (err) {
                console.log(" LIBNMAP -> generating report(FROM PROC ERROR), callback should be called in here");
          ...
          execute.stdout.on('end', function bytesend() {
            if (report.length > 0) {
              console.log(" LIBNMAP -> generating report (FROM PROC STDOUT END), callback should be called in here");
